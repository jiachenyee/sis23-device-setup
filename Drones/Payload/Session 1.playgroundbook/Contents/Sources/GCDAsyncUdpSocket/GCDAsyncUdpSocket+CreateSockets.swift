...
extension GCDAsyncUdpSocket {
    ...
    func createSocket6() throws {
        let socketFD = try GCDAsyncUdpSocket.createSocketByVersion(family: AF_INET6)
        /**
         ...
         **/
        var maxSendSize: UInt = UInt(self._maxSendSize)
        var status = setsockopt(socketFD, SOL_SOCKET, SO_SNDBUF, &maxSendSize, socklen_t(MemoryLayout.size(ofValue: maxSendSize)))
        if (status == -1){
            Darwin.close(socketFD)
        ...
        status = setsockopt(socketFD, SOL_SOCKET, SO_RCVBUF, &max6ReceiveSize, socklen_t(MemoryLayout.size(ofValue: max6ReceiveSize)))
